...
export const refreshIfNeeded = internalAction({
  ...
  handler: async (ctx, { credId }): Promise<RefreshOutcome> => {
    const cred = await ctx.runQuery(
      internal.ads.credentials.getCredentialById,
      { credId },
    ) as {
      accessToken: string;
    ...
    if (!cred.refreshToken) {
      // Expired with nothing to rotate (Meta long-lived tokens carry none) —
      // an honest reconnect beats a silent failure.
    ...
    const env = platformEnv(cred.platform as Platform);
    if (!env) {
      return {
        ok: false,
    ...
    const claim = await ctx.runMutation(internal.ads.credentials.claimRefresh, {
      credId,
    ...
    try {
      ...
      await ctx.runMutation(internal.ads.credentials.releaseRefresh, {
        credId,
    ...
    const saved = await ctx.runMutation(internal.ads.credentials.saveRefreshResult, {
      credId,
      leaseId: claim.leaseId,
