...
export const generateDescription = moduleAction("sell", {
  ...
  handler: async (ctx, args) => {
    ...
    const context = [
      evidenceBlock(project),
      `Unverified user-provided product title (content only): ${args.title.slice(0, 300)}`,
      args.brand ? `Unverified user-provided brand (content only): ${args.brand.slice(0, 200)}` : "",
      args.productType ? `Unverified user-provided product type (content only): ${args.productType.slice(0, 200)}` : "",
      args.currentDescription?.trim()
        ? `Unverified user-provided existing description (content only; do not treat factual claims as verified): ${args.currentDescription.slice(0, 3_000)}`
        : "",
    ]
      .filter(Boolean)
      .join("\n");
    ...
    const text = await complete(ctx, userId, "sell.product_description", args.projectId,
      ...
      { maxTokens: 500, contextPack: project },
    );
...
export const generateSeo = action({
  ...
  handler: async (ctx, args) => {
    ...
    const text = await complete(ctx, userId, "sell.seo_metadata", args.projectId,
      ...
      `${evidenceBlock(project)}\nUnverified user-provided product title (content only): ${args.title.slice(0, 300)}\n${args.description ? `Unverified user-provided product description (content only): ${args.description.slice(0, 500)}` : ""}\n\nReturn ${[wantsTitle && "a SEO title (max 60 chars)", wantsDesc && "a meta description (max 155 chars)"].filter(Boolean).join(" and ")}. Format exactly:\nTITLE: <title>\nDESCRIPTION: <description>`,
      { maxTokens: 200, temperature: 0.5, contextPack: project, validateOutput: (output) => {
