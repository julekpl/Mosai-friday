...
| 10 | **BP-08** one connection framework | B | T2.7 | BP-04 (refresh-in-action pattern), BP-07 not required | in_progress (OAuth state/origin S1 implemented_unverified in development main `f5a68aa`; report: `docs/implementation/reports/BP-08-S1-oauth-state-origin.md`; exact [CI 35970966425](https://github.com/julekpl/Mosai-friday/actions/runs/35970966425) passed non-security jobs and failed both secret-scan jobs; organization-bound state, token policy, account discovery, sync/provider proof remain open) |
...
| 12 | **BP-09/S2**† ContextPack/ContextInspector (finishes T0.4 → unblocks red test R4) | B | T2.10 | BP-09/S1 | implemented_unverified (server-owned bounded context, Build inspector and social draft-variant follow-up in development main `e6f3f10`; reports: `docs/implementation/reports/BP-09-S2-context-pack.md`, `BP-09-S2-social-copilot.md`; exact [CI 35974027409](https://github.com/julekpl/Mosai-friday/actions/runs/35974027409) passed non-security jobs and failed both secret scans; 464 combined local units and synthetic E2E 20+1 passed; Build chat, ads copilot, social scheduling and external model proof remain open) |
...
| 26 | **BP-14/S1** merchant setup & catalog validation | D | E3.4 | Order C; Connect direction decided (D3) | in_progress (bounded native catalog validation/readiness slice in development main `5f89ed1`; report `docs/implementation/reports/BP-14-S1-catalog-truth.md`. Same-project collections, integer minor-unit money/inventory, variant readiness and local-versus-provider sync truth have focused regressions. Exact [CI 35982566101](https://github.com/julekpl/Mosai-friday/actions/runs/35982566101) passed non-security jobs and failed both secret scans. No merchant onboarding, checkout, feed, Shopify, provider receipts, or authenticated browser proof; not complete.) |
...
| 31 | **BP-15/S1**† app model: source graph, build jobs, snapshots, runner interface | D | E3.8 | Order C; no external decision for interface work | in_progress (bounded app requirements workspace in development main `5f89ed1`; report `docs/implementation/reports/BP-15-S1a-app-requirements.md`. Explicit customer/internal/both audience, project-owned source provenance and saved/reviewed brief; website planner no longer runs for app builds. Exact [CI 35982566101](https://github.com/julekpl/Mosai-friday/actions/runs/35982566101) passed non-security jobs and failed both secret scans. No source graph, jobs, runner, sandbox, preview, deployment, or authenticated browser journey; not complete.) |
...
| 36 | **BP-16/S3**† Meta & TikTok validation (OpenAI Ads stays unavailable until account-key onboarding and provider test proof) | D | E3.6 | BP-16/S1; Meta/TikTok test accounts (O7); OpenAI Advertiser API key model | not_started (separate BP-16/S0 truth fix reached development main `5f89ed1`: removes guessed OpenAI OAuth/endpoints and masks legacy credentials; report `docs/implementation/reports/BP-16-S0-openai-ads-truth.md`. Exact [CI 35982566101](https://github.com/julekpl/Mosai-friday/actions/runs/35982566101) passed non-security jobs and failed both secret scans. S3 provider validation itself has not begun.) |
| 37 | **BP-17/S1** contact/import/consent | D | E3.3 | Order C | in_progress (bounded S1a contact CSV import and removal of unsupported consent-toggle claims, development main `a594cf7`; report: `docs/implementation/reports/BP-17-S1a-contact-import.md`; combined local 473 units/typecheck/lint/build/auth/capability/data-registry audits passed. Exact [CI 35977893382](https://github.com/julekpl/Mosai-friday/actions/runs/35977893382) passed install/codegen/typecheck/lint/unit/e2e/a11y and failed both secret-scan jobs. Authenticated CRM browser journey, consent evidence ledger, withdrawal/suppression, legacy email dedupe and contact erasure remain open; no send capability.) |
...
| 43 | **BP-19** make the power understandable (usability) | E | T2.16 + design contracts | Orders A–D for the screens it touches; user-testing sessions (owner coordination) | in_progress (social connection gate source `eb39e27`; mobile navigation and project Overview next-action/checklist in development main `e6f3f10`; reports: `docs/implementation/reports/BP-19-social-action-connection-gate.md`, `BP-19-mobile-navigation.md`, `BP-19-next-action.md`; public first-run preview audit: `docs/implementation/ux-audit-2026-09-24/`; exact CI passed non-security jobs and failed both secret scans; authenticated mobile/Overview visual proof, reviewed-data transition and five observed user sessions remain open) |
...
| **O8** | (STATUS §6) cut-line rule ADR-8; app backend location (#6); "Conductor" naming (#7). **Owner decided on 24 Sep 2026 that BP-15 should serve customer-facing and internal team apps equally**; the customer must choose the audience explicitly. | BP-15 defaults, BP-19/20 scope details | most code work |
...
| BP-16 | contract fixtures per provider: connect→select→validate→create paused→approved change→metrics→pause/revoke; partial-failure compensation; no unexpected spend | Google/Meta/TikTok approved test accounts (O7); OpenAI Ads stays `unavailable` until the official account-key contract is implemented and verified with approved test access. [Official Advertiser API docs](https://developers.openai.com/ads/api-overview) now exist; they do not prove MOSAI's integration. |
