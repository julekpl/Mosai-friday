...
| 26 | **BP-14/S1** merchant setup & catalog validation | D | E3.4 | Order C; Connect direction decided (D3) | in_progress (bounded native catalog validation/readiness slice local in `bp-02-codex`; report `docs/implementation/reports/BP-14-S1-catalog-truth.md`. Same-project collections, integer minor-unit money/inventory, variant readiness and local-versus-provider sync truth have focused regressions. No merchant onboarding, checkout, feed, Shopify, provider receipts, or authenticated browser proof; not complete.) |
...
| 31 | **BP-15/S1**† app model: source graph, build jobs, snapshots, runner interface | D | E3.8 | Order C; no external decision for interface work | in_progress (bounded app requirements workspace local in `bp-02-codex`; report `docs/implementation/reports/BP-15-S1a-app-requirements.md`. Explicit customer/internal/both audience, project-owned source provenance and saved/reviewed brief; website planner no longer runs for app builds. No source graph, jobs, runner, sandbox, preview, deployment, or authenticated browser journey; not complete.) |
...
| 36 | **BP-16/S3**† Meta & TikTok validation (OpenAI Ads stays unavailable until account-key onboarding and provider test proof) | D | E3.6 | BP-16/S1; Meta/TikTok test accounts (O7); OpenAI Advertiser API key model | not_started (separate BP-16/S0 truth fix local in `bp-02-codex`: removes guessed OpenAI OAuth/endpoints and masks legacy credentials; report `docs/implementation/reports/BP-16-S0-openai-ads-truth.md`. S3 provider validation itself has not begun.) |
