...
| 5 | **BP-05** export/deletion lifecycle | B | T2.5 | Order A; decisions already recorded (D2) | implemented_unverified (local `0fde75c`, report: `docs/implementation/reports/BP-05-export-deletion-lifecycle.md`; 30-day durable lifecycle, registry-driven export/deletion, opaque cursors, fair durable jobs, active-membership checks, guarded multi-org cleanup and bounded obligation inspection, per-step export reauthorization and current-access chunk reads, cancellation/retry; backend export endpoints exist but customer self-service export/download UI remains; 375 source-branch unit tests pass; exact integrated checks and CI pending; secret scan remains blocked by known D-01/T0.1/B3; codegen needs a development deployment; provider behavior unverified) |
...
| 9 | **BP-07** jobs, events, receipts, approvals | B | T2.6, T2.12 | BP-05 not required; schema/crons exist | in_progress (ads claim/receipt S1 `db64359` implemented_unverified; report: `docs/implementation/reports/BP-07-S1-ads-execution-claim.md`; lease/reconciliation, events, approvals and provider proof remain open) |
...
| 43 | **BP-19** make the power understandable (usability) | E | T2.16 + design contracts | Orders A–D for the screens it touches; user-testing sessions (owner coordination) | in_progress (social connection gate slice `eb39e27` implemented_unverified; report: `docs/implementation/reports/BP-19-social-action-connection-gate.md`; full journeys and user sessions remain open) |
...
| **O3** | Owner selected **EU/EEA, multilingual first**, with launch languages in priority order: English, German, French, Italian, Spanish, Polish, Dutch, Romanian, Greek, Hungarian, Czech, Swedish, Portuguese, Bulgarian, Danish. Specific EU data-processing and residency requirements remain open. | capability matrix fill (STATUS §5 #4), verified localization claims, BP-20 residency rows | locale-ready UI, provider capability matrix, data inventory |
