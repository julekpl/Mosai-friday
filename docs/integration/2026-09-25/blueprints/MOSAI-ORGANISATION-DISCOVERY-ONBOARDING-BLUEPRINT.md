...
>
> - **Adopted now:** the six-screen first run adopted on 26 Sep (PRs #25-#27, plan section 11) is the onboarding; discovery, when built, pre-fills its answers and its "Here's what we found" step and never adds a screen; the goal options are the multi-select list shipped with that first run; the Google listing search becomes an explicit button under a SerpApi ceiling (LQ-1).
