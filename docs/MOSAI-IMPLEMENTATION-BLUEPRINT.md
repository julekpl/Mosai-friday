...
For Meta and TikTok, validate current API versions, fields, token exchange, scopes, account access, units and rate-limit behavior through contract fixtures plus approved test accounts. OpenAI now publishes an [Advertiser API overview](https://developers.openai.com/ads/api-overview) and [authentication contract](https://developers.openai.com/ads/api-reference/authentication): an account-scoped server-held API key, not the repository's guessed OAuth flow. Treat ChatGPT/OpenAI Ads as unavailable until the credential onboarding model, account access and each advertised operation are verified with approved test credentials; delete guessed endpoints rather than ship them as support. Documentation alone does not establish MOSAI's access or operational readiness.
