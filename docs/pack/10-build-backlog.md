...
| T2.17 | Tool calling in `ModelGateway` `[N][R]` (in-product agent decision, 24 Sep 2026) | OpenRouter tool-call request and response shape; bounded loop with a hard step cap; arguments validated by schema; unknown tools refused; tools limited to `read` and `draft` effects; tenant ids never model arguments; one `aiRuns` row per step | A model cannot call an unregistered tool, pass an invalid argument to a handler, or exceed the step cap; every step has a run record | T2.9 (gateway exists) |
| T2.18 | Copilot tool registry `[N][R]` | `src/shared` registry of read, navigate and draft tools, each bound to a capability (`<module>.<action>`) and calling the same guarded internal function the UI uses; project-wide read built on `contextPack` | A tool for a locked module reports `locked`; a cross-tenant id returns nothing; no tool has a publish/send/spend effect | T2.17, T2.3 |
| T2.19 | Copilot threads and app-wide dock `[N][M]` | Project-scoped thread table (registered in the data registry); one action per user turn consuming one quota unit; dock in `AppShell` that knows the current route; tool calls and links rendered with status UI | Threads export and delete with the project; a turn costs one quota unit; keyboard and screen-reader reachable | T2.18, T2.5 |
| T2.20 | Next-step model for all eight modules `[N]` | Extend `next-action-model.ts` from three modules to eight, driven by capability state and review state rather than row counts; shared with the copilot | Saved-but-unreviewed work is never shown as done | T2.3 |
