...
**CI reality check (independent read-only verification, 23 Sep 2026):** GitHub
`main` = `d3f6a8e09e301944789dc2c4487a831dc7698137`; run
[`35828575954`](https://github.com/julekpl/Mosai-friday/actions/runs/35828575954)
= **completed / failure**. typecheck, lint, unit, codegen drift, e2e and a11y
**passed on that exact commit**; **both security jobs failed** at their
secret-scan steps (`security`: the tracked `.env.keys`; `security-history`:
a historical credential — values redacted, never reproduced). The BP-01 split
...
**CI confirmation (run `35828575954`, commit `d3f6a8e`, 23 Sep 2026):** the
...
findings above, now proven on the real runner instead of inferred. Neither
